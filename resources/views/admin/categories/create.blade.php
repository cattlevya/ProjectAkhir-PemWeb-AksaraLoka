...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <h1 class="font-display text-3xl text-espresso-deep mb-8">{{ isset($category) ? 'Edit Kategori' : 'Kategori Baru' }}</h1>
    <form action="{{ isset($category) ? route('admin.categories.update', $category) : route('admin.categories.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="slug" value="{{ old('slug', $category->slug ?? '') }}" class="w-full bg-cream-warm border border-cream-mid px-4 py-3 text-sm focus:border-gold focus:ring-0" placeholder="Auto-generate dari nama">
            </div>
            <div>
